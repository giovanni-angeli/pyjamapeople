...
class HttpHandler(tornado.web.RequestHandler):  # pylint: disable=too-few-public-methods, abstract-method
    ...
    def get(self):
        ...
        ctx = {
            ...
            'backend_callables': self.parent.backend_callables,
        }
...
class WebsockHandler(tornado.websocket.WebSocketHandler):  # pylint: disable=abstract-method
    ...
    def on_message(self, message):
        ...
        if self.parent:
            t_ = self.parent._handle_message_from_front(self, message)
...
class Positron:
    ...
    async def _handle_message_from_front(self, ws_socket, data):
        ...
        try:
            ...
            _args = data.get('args', {})
            logging.info(f"data:{data}, _args:{_args}")

            if self.backend_instance and hasattr(self.backend_instance, _callable):
                ret = await getattr(self.backend_instance, _callable)(ws_socket, **_args)
                if ret:
